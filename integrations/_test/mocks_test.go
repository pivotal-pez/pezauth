...
func setMongoEnv(ip, port string) {
	...
	vcap_services := `{"p-mongodb": [	{"name": "portal-mongo","label": "p-mongodb","tags": ["pivotal","mongodb"],"plan": "development","credentials": {"database":"59501f9b-5528-421b-84e9-93f9dc6f1080","uri": "{{.URI}}"}}]}	`
	mongoTmpl, _ := template.New("mongoVcap").Parse(vcap_services)
	var b bytes.Buffer
	mongoTmpl.Execute(&b, mockVcap)
	os.Setenv("VCAP_SERVICES", b.String())
	os.Setenv("MONGO_SERVICE_NAME", "portal-mongo")
	os.Setenv("MONGO_URI_NAME", "uri")
	os.Setenv("MONGO_COLLECTION_NAME", "org_users")
	os.Setenv("MONGO_DB_NAME", "database")
}
