...
var _ = Describe("MyRedis", func() {
	...
	BeforeEach(func() {
		...
		rdis.Pool.Get().Do("DEL", key)
		rdis.Pool.Get().Do("DEL", hash)
	})

	Context("Calling .New", func() {
		It("Should return a valid redis session", func() {
			rdis := new(integrations.MyRedis).New(appEnv)
			Ω(rdis.Pool.Get().Err()).Should(BeNil())
		})
	})
	...
	Context("Keys", func() {
		Context("Calling SET for a key/value", func() {
			It("Should not error", func() {
				rdis := new(integrations.MyRedis).New(appEnv)
				s, e := rdis.Pool.Get().Do("SET", key, val)
				Ω(s).Should(Equal("OK"))
				Ω(e).Should(BeNil())
			})
		})
		...
		Context("Calling GET on a key", func() {
			It("Should return the key's value", func() {
				rdis := new(integrations.MyRedis).New(appEnv)
				rdis.Pool.Get().Do("SET", key, val)
				o, e := rdis.Pool.Get().Do("GET", key)
		...
		Context("Calling KEYS with patter", func() {
			It("Should find keys that match", func() {
				rdis := new(integrations.MyRedis).New(appEnv)
				rdis.Pool.Get().Do("SET", key, val)
				o, e := rdis.Pool.Get().Do("KEYS", key[0:len(key)-1]+"*")
				s, _ := o.([]interface{})
	...
	Context("Hashes", func() {
		Context("Calling HMSET for a hash with key/value", func() {
			It("Should not error", func() {
				rdis := new(integrations.MyRedis).New(appEnv)
				s, e := rdis.Pool.Get().Do("HMSET", hash, hashSubkey, hashSubval)
				Ω(s).Should(Equal("OK"))
				Ω(e).Should(BeNil())
			})
		})
		...
		Context("Calling HMGET for a hash with a given key", func() {
			It("Should return stored value for that key", func() {
				rdis := new(integrations.MyRedis).New(appEnv)
				rdis.Pool.Get().Do("HMSET", hash, hashSubkey, hashSubval)
				o, e := rdis.Pool.Get().Do("HMGET", hash, hashSubkey)
				Ω(e).Should(BeNil())
				s, _ := o.([]interface{})
